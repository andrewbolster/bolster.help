...
Deploys do not go through `wrangler deploy`: the API token cannot read
`/accounts/{id}/workers/subdomain`, which wrangler queries on every deploy. The
Worker is uploaded with a multipart `PUT` to
`/accounts/{id}/workers/scripts/bolster-help` instead. `new_sqlite_classes` may
only be applied once — replaying it fails with `10074`, because objects already
depend on the class.
...
**Tool results are CLI-formatted bulk data**, clipped at 2,000 characters before
re-entering the loop. A question like "how many births last year" gets the head
of a 741-row CSV whose summary line reads like an answer — which is how the
model once reported the row count as a birth count.
